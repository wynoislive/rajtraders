...
async function seed() {
  console.log('🌸 Seeding Sundarvan Cakes & Party Decoration products into PostgreSQL database...');
  for (const item of celebrationProducts) {
    ...
    if (existing.length === 0) {
      ...
      await db.update(productsTable).set({ ...item, updatedAt: new Date() }).where(eq(productsTable.id, item.id));
